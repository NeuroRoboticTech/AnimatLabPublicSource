...
class tree {
	...
	public:
      ...
      sibling_iterator move_before(sibling_iterator target, sibling_iterator source);
		...
		/// Move 'source' node (plus its children) to become the child of 'target'.
...
template <class T, class tree_node_allocator>
template <typename iter> iter tree<T, tree_node_allocator>::move_below(iter target, iter source)
	{
	tree_node *dst=target.node;
	tree_node *src=source.node;
	assert(dst);
	...
	// connect it to the new point
	src->parent=target.node;
	if(target.node->last_child!=0) {
		target.node->last_child->next_sibling=src;
		}
	else {
		target.node->first_child=src;
		}
	src->prev_sibling=target.node->last_child;
	target.node->last_child=src;
	src->next_sibling=0;
	return src;
	}
...
#pragma region ptr_tree_code


/// A node in the ptr_tree, combining links to other nodes as well as the actual data.
template<class T>
class ptr_tree_node_ { // size: 5*4=20 bytes (on 32 bit arch), can be reduced by 8.
	public:
		ptr_tree_node_<T> *parent;
	   ptr_tree_node_<T> *first_child, *last_child;
		ptr_tree_node_<T> *prev_sibling, *next_sibling;
		T data;
}; // __attribute__((packed));

template <class T, class ptr_tree_node_allocator = std::allocator<ptr_tree_node_<T> > >
class ptr_tree {
	protected:
		typedef ptr_tree_node_<T> ptr_tree_node;
	public:
		/// Value of the data stored at a node.
		typedef T value_type;
      ...
      class leaf_iterator;
		...
		ptr_tree();
		ptr_tree(const T&);
		ptr_tree(const iterator_base&);
		ptr_tree(const ptr_tree<T, ptr_tree_node_allocator>&);
		~ptr_tree();
		void operator=(const ptr_tree<T, ptr_tree_node_allocator>&);
      ...
      /// Base class for iterators, only pointers stored, no traversal logic.
		...
		class pre_order_iterator : public iterator_base { 
			public:
				pre_order_iterator();
				pre_order_iterator(ptr_tree_node *);
				pre_order_iterator(const iterator_base&);
				pre_order_iterator(const sibling_iterator&);
		...
		class post_order_iterator : public iterator_base {
			public:
				post_order_iterator();
				post_order_iterator(ptr_tree_node *);
				post_order_iterator(const iterator_base&);
				post_order_iterator(const sibling_iterator&);
			   ...
			   post_order_iterator&  operator--();
				...
				/// Set iterator to the first child as deep as possible down the ptr_tree.
				void descend_all();
		};
		...
		class breadth_first_queued_iterator : public iterator_base {
			public:
				breadth_first_queued_iterator();
				breadth_first_queued_iterator(ptr_tree_node *);
				breadth_first_queued_iterator(const iterator_base&);
			...
			private:
				std::queue<ptr_tree_node *> traversal_queue;
		};

		/// The default iterator types throughout the ptr_tree class.
		typedef pre_order_iterator            iterator;
		typedef breadth_first_queued_iterator breadth_first_iterator;
		...
		class fixed_depth_iterator : public iterator_base {
			public:
				fixed_depth_iterator();
				fixed_depth_iterator(ptr_tree_node *);
				fixed_depth_iterator(const iterator_base&);
				fixed_depth_iterator(const sibling_iterator&);
				fixed_depth_iterator(const fixed_depth_iterator&);
		...
		class sibling_iterator : public iterator_base {
			public:
				sibling_iterator();
				sibling_iterator(ptr_tree_node *);
				sibling_iterator(const sibling_iterator&);
				sibling_iterator(const iterator_base&);
				...
				ptr_tree_node *range_first() const;
				ptr_tree_node *range_last() const;
				ptr_tree_node *parent_;
			private:
				void set_parent_();
		};
      ...
      class leaf_iterator : public iterator_base {
         public:
            leaf_iterator();
            leaf_iterator(ptr_tree_node *, ptr_tree_node *top=0);
            leaf_iterator(const sibling_iterator&);
            leaf_iterator(const iterator_base&);
		...
		/// Return iterator to the beginning of the ptr_tree.
		inline pre_order_iterator   begin() const;
		/// Return iterator to the end of the ptr_tree.
		inline pre_order_iterator   end() const;
		/// Return post-order iterator to the beginning of the ptr_tree.
		post_order_iterator  begin_post() const;
		/// Return post-order end iterator of the ptr_tree.
		post_order_iterator  end_post() const;
		/// Return fixed-depth iterator to the first node at a given depth from the given iterator.
		fixed_depth_iterator begin_fixed(const iterator_base&, unsigned int) const;
		/// Return fixed-depth end iterator.
		...
		/// Return sibling end iterator for children of given node.
		sibling_iterator     end(const iterator_base&) const;
      /// Return leaf iterator to the first leaf of the ptr_tree.
      leaf_iterator   begin_leaf() const;
      /// Return leaf end iterator for entire ptr_tree.
      leaf_iterator   end_leaf() const;
      /// Return leaf iterator to the first leaf of the subptr_tree at the given node.
      leaf_iterator   begin_leaf(const iterator_base& top) const;
      /// Return leaf end iterator for the subptr_tree at the given node.
      leaf_iterator   end_leaf(const iterator_base& top) const;
		...
		/// Erase all nodes of the ptr_tree.
		void     clear();
		/// Erase element at position pointed to by iterator, return incremented iterator.
		template<typename iter> iter erase(iter);
		/// Erase all children of the node pointed to by iterator.
		...
		/// Short-hand to insert topmost node in otherwise empty ptr_tree.
		pre_order_iterator set_head(const T& x);
		/// Insert node as previous sibling of node pointed to by position.
		template<typename iter> iter insert(iter position, const T& x);
		/// Specialisation of previous member.
      ...
      sibling_iterator move_before(sibling_iterator target, sibling_iterator source);
		...
		bool     equal_subtree(const iter& one, const iter& two, BinaryPredicate) const;
		/// Extract a new tree formed by the range of siblings plus all their children.
		ptr_tree     subtree(sibling_iterator from, sibling_iterator to) const;
		void     subtree(ptr_tree&, sibling_iterator from, sibling_iterator to) const;
		/// Exchange the node (plus subtree) with its sibling node (do nothing if no sibling present).
		void     swap(sibling_iterator it);
		/// Exchange two nodes (plus subtrees)
	   void     swap(iterator, iterator);
		...
		class iterator_base_less {
			public:
				bool operator()(const typename ptr_tree<T, ptr_tree_node_allocator>::iterator_base& one,
									 const typename ptr_tree<T, ptr_tree_node_allocator>::iterator_base& two) const
					{
					return one.node < two.node;
					}
		};
		ptr_tree_node *head, *feet;    // head/feet are always dummy; if an iterator points to them it is invalid
	private:
		ptr_tree_node_allocator alloc_;
		void head_initialise_();
		void copy_(const ptr_tree<T, ptr_tree_node_allocator>& other);
      ...
      /// Comparator class for two nodes of a tree (used for sorting and searching).
		...
		class compare_nodes {
			public:
				...
				bool operator()(const ptr_tree_node *a, const ptr_tree_node *b) 
					{
					return comp_(a->data, b->data);
					}
			private:
...
//template <class T, class ptr_tree_node_allocator>
//class iterator_base_less {
//	public:
//		bool operator()(const typename ptr_tree<T, ptr_tree_node_allocator>::iterator_base& one,
//						  const typename ptr_tree<T, ptr_tree_node_allocator>::iterator_base& two) const
...
// template <class T, class ptr_tree_node_allocator>
// bool operator<(const typename ptr_tree<T, ptr_tree_node_allocator>::iterator& one,
// 					const typename ptr_tree<T, ptr_tree_node_allocator>::iterator& two)
// 	{
// 	txtout << "operator< " << one.node < two.node << std::endl;
// 	if(one.node < two.node) return true;
// 	return false;
...
// template <class T, class ptr_tree_node_allocator>
// bool operator>(const typename ptr_tree<T, ptr_tree_node_allocator>::iterator_base& one,
// 					const typename ptr_tree<T, ptr_tree_node_allocator>::iterator_base& two)
// 	{
// 	txtout << "operator> " << one.node < two.node << std::endl;
// 	if(one.node > two.node) return true;
// 	return false;
...
template <class T, class ptr_tree_node_allocator>
ptr_tree<T, ptr_tree_node_allocator>::ptr_tree() 
	{
	head_initialise_();
	}

template <class T, class ptr_tree_node_allocator>
ptr_tree<T, ptr_tree_node_allocator>::ptr_tree(const T& x) 
	{
	head_initialise_();
	set_head(x);
	}
...
template <class T, class ptr_tree_node_allocator>
ptr_tree<T, ptr_tree_node_allocator>::ptr_tree(const iterator_base& other)
	{
	head_initialise_();
	set_head((*other));
	replace(begin(), other);
...
template <class T, class ptr_tree_node_allocator>
void ptr_tree<T, ptr_tree_node_allocator>::head_initialise_() 
   { 
   head = alloc_.allocate(1,0); // MSVC does not have default second argument 
	feet = alloc_.allocate(1,0);
...
template <class T, class ptr_tree_node_allocator>
void ptr_tree<T, ptr_tree_node_allocator>::operator=(const ptr_tree<T, ptr_tree_node_allocator>& other)
	{
	copy_(other);
	}

template <class T, class ptr_tree_node_allocator>
ptr_tree<T, ptr_tree_node_allocator>::ptr_tree(const ptr_tree<T, ptr_tree_node_allocator>& other)
	{
	head_initialise_();
	copy_(other);
	}
...
template <class T, class ptr_tree_node_allocator>
void ptr_tree<T, ptr_tree_node_allocator>::copy_(const ptr_tree<T, ptr_tree_node_allocator>& other) 
	{
	clear();
	pre_order_iterator it=other.begin(), to=begin();
	while(it!=other.end()) {
...
template <class T, class ptr_tree_node_allocator>
void ptr_tree<T, ptr_tree_node_allocator>::clear()
	{
	if(head)
		while(head->next_sibling!=feet)
			erase(pre_order_iterator(head->next_sibling));
...
template<class T, class ptr_tree_node_allocator> 
void ptr_tree<T, ptr_tree_node_allocator>::erase_children(const iterator_base& it)
	{
//	std::cout << "erase_children " << it.node << std::endl;
	if(it.node==0) return;

	ptr_tree_node *cur=it.node->first_child;
	ptr_tree_node *prev=0;

	while(cur!=0) {
		prev=cur;
		cur=cur->next_sibling;
		erase_children(pre_order_iterator(prev));
		kp::ptr_destructor(&prev->data);
		alloc_.deallocate(prev,1);
		}
	it.node->first_child=0;
	it.node->last_child=0;
...
template<class T, class ptr_tree_node_allocator> 
template<class iter>
iter ptr_tree<T, ptr_tree_node_allocator>::erase(iter it)
	{
	ptr_tree_node *cur=it.node;
	assert(cur!=head);
	iter ret=it;
	ret.skip_children();
	++ret;
	...
	kp::ptr_destructor(&cur->data);
   alloc_.deallocate(cur,1);
	return ret;
	}

template <class T, class ptr_tree_node_allocator>
typename ptr_tree<T, ptr_tree_node_allocator>::pre_order_iterator ptr_tree<T, ptr_tree_node_allocator>::begin() const
	{
	return pre_order_iterator(head->next_sibling);
	}

template <class T, class ptr_tree_node_allocator>
typename ptr_tree<T, ptr_tree_node_allocator>::pre_order_iterator ptr_tree<T, ptr_tree_node_allocator>::end() const
	{
	return pre_order_iterator(feet);
	}

template <class T, class ptr_tree_node_allocator>
typename ptr_tree<T, ptr_tree_node_allocator>::breadth_first_queued_iterator ptr_tree<T, ptr_tree_node_allocator>::begin_breadth_first() const
	{
	return breadth_first_queued_iterator(head->next_sibling);
	}

template <class T, class ptr_tree_node_allocator>
typename ptr_tree<T, ptr_tree_node_allocator>::breadth_first_queued_iterator ptr_tree<T, ptr_tree_node_allocator>::end_breadth_first() const
	{
	return breadth_first_queued_iterator();
	}

template <class T, class ptr_tree_node_allocator>
typename ptr_tree<T, ptr_tree_node_allocator>::post_order_iterator ptr_tree<T, ptr_tree_node_allocator>::begin_post() const
	{
	ptr_tree_node *tmp=head->next_sibling;
	if(tmp!=feet) {
		while(tmp->first_child)
			tmp=tmp->first_child;
		}
...
template <class T, class ptr_tree_node_allocator>
typename ptr_tree<T, ptr_tree_node_allocator>::post_order_iterator ptr_tree<T, ptr_tree_node_allocator>::end_post() const
	{
	return post_order_iterator(feet);
	}

template <class T, class ptr_tree_node_allocator>
typename ptr_tree<T, ptr_tree_node_allocator>::fixed_depth_iterator ptr_tree<T, ptr_tree_node_allocator>::begin_fixed(const iterator_base& pos, unsigned int dp) const
	{
	typename ptr_tree<T, ptr_tree_node_allocator>::fixed_depth_iterator ret;
	ret.top_node=pos.node;

	ptr_tree_node *tmp=pos.node;
	unsigned int curdepth=0;
	while(curdepth<dp) { // go down one level
		while(tmp->first_child==0) {
			if(tmp->next_sibling==0) {
				...
				do {
					if(tmp==ret.top_node)
					   throw std::range_error("ptr_tree: begin_fixed out of range");
					tmp=tmp->parent;
               if(tmp==0) 
					   throw std::range_error("ptr_tree: begin_fixed out of range");
               --curdepth;
...
template <class T, class ptr_tree_node_allocator>
typename ptr_tree<T, ptr_tree_node_allocator>::fixed_depth_iterator ptr_tree<T, ptr_tree_node_allocator>::end_fixed(const iterator_base& pos, unsigned int dp) const
	{
	assert(1==0); // FIXME: not correct yet: use is_valid() as a temporary workaround 
	ptr_tree_node *tmp=pos.node;
	unsigned int curdepth=1;
	while(curdepth<dp) { // go down one level
		while(tmp->first_child==0) {
			tmp=tmp->next_sibling;
			if(tmp==0)
				throw std::range_error("ptr_tree: end_fixed out of range");
			}
		tmp=tmp->first_child;
		++curdepth;
		}
...
template <class T, class ptr_tree_node_allocator>
typename ptr_tree<T, ptr_tree_node_allocator>::sibling_iterator ptr_tree<T, ptr_tree_node_allocator>::begin(const iterator_base& pos) const
	{
	assert(pos.node!=0);
	if(pos.node->first_child==0) {
		return end(pos);
...
template <class T, class ptr_tree_node_allocator>
typename ptr_tree<T, ptr_tree_node_allocator>::sibling_iterator ptr_tree<T, ptr_tree_node_allocator>::end(const iterator_base& pos) const
	{
	sibling_iterator ret(0);
	ret.parent_=pos.node;
	return ret;
...
template <class T, class ptr_tree_node_allocator>
typename ptr_tree<T, ptr_tree_node_allocator>::leaf_iterator ptr_tree<T, ptr_tree_node_allocator>::begin_leaf() const
   {
   ptr_tree_node *tmp=head->next_sibling;
   if(tmp!=feet) {
      while(tmp->first_child)
...
template <class T, class ptr_tree_node_allocator>
typename ptr_tree<T, ptr_tree_node_allocator>::leaf_iterator ptr_tree<T, ptr_tree_node_allocator>::end_leaf() const
   {
   return leaf_iterator(feet);
   }

template <class T, class ptr_tree_node_allocator>
typename ptr_tree<T, ptr_tree_node_allocator>::leaf_iterator ptr_tree<T, ptr_tree_node_allocator>::begin_leaf(const iterator_base& top) const
   {
	ptr_tree_node *tmp=top.node;
	while(tmp->first_child)
		 tmp=tmp->first_child;
   return leaf_iterator(tmp, top.node);
   }
...
template <class T, class ptr_tree_node_allocator>
typename ptr_tree<T, ptr_tree_node_allocator>::leaf_iterator ptr_tree<T, ptr_tree_node_allocator>::end_leaf(const iterator_base& top) const
   {
   return leaf_iterator(top.node, top.node);
   }
...
template <class T, class ptr_tree_node_allocator>
template <typename iter>
iter ptr_tree<T, ptr_tree_node_allocator>::parent(iter position) 
	{
	assert(position.node!=0);
	return iter(position.node->parent);
	}
...
template <class T, class ptr_tree_node_allocator>
template <typename iter>
iter ptr_tree<T, ptr_tree_node_allocator>::previous_sibling(iter position) const
	{
	assert(position.node!=0);
	iter ret(position);
	ret.node=position.node->prev_sibling;
...
template <class T, class ptr_tree_node_allocator>
template <typename iter>
iter ptr_tree<T, ptr_tree_node_allocator>::next_sibling(iter position) const
	{
	assert(position.node!=0);
	iter ret(position);
	ret.node=position.node->next_sibling;
...
template <class T, class ptr_tree_node_allocator>
template <typename iter>
iter ptr_tree<T, ptr_tree_node_allocator>::next_at_same_depth(iter position) const
	{
	// We make use of a temporary fixed_depth iterator to implement this.

	typename ptr_tree<T, ptr_tree_node_allocator>::fixed_depth_iterator tmp(position.node);
...
template <class T, class ptr_tree_node_allocator>
template <typename iter>
iter ptr_tree<T, ptr_tree_node_allocator>::append_child(iter position)
 	{
	assert(position.node!=head);
	assert(position.node);

	ptr_tree_node *tmp=alloc_.allocate(1,0);
	kp::constructor(&tmp->data);
	tmp->first_child=0;
	tmp->last_child=0;
...
template <class T, class ptr_tree_node_allocator>
template <typename iter>
iter ptr_tree<T, ptr_tree_node_allocator>::prepend_child(iter position)
 	{
	assert(position.node!=head);
	assert(position.node);
...
template <class T, class ptr_tree_node_allocator>
template <class iter>
iter ptr_tree<T, ptr_tree_node_allocator>::append_child(iter position, const T& x)
	{
	// If your program fails here you probably used 'append_child' to add the top
	// node to an empty ptr_tree. From version 1.45 the top element should be added
	// using 'insert'. See the documentation for further information, and sorry about
	// the API change.
	assert(position.node!=head);
	assert(position.node);
...
template <class T, class ptr_tree_node_allocator>
template <class iter>
iter ptr_tree<T, ptr_tree_node_allocator>::prepend_child(iter position, const T& x)
	{
	assert(position.node!=head);
	assert(position.node);
...
template <class T, class ptr_tree_node_allocator>
template <class iter>
iter ptr_tree<T, ptr_tree_node_allocator>::append_child(iter position, iter other)
	{
	assert(position.node!=head);
	assert(position.node);
...
template <class T, class ptr_tree_node_allocator>
template <class iter>
iter ptr_tree<T, ptr_tree_node_allocator>::prepend_child(iter position, iter other)
	{
	assert(position.node!=head);
	assert(position.node);
...
template <class T, class ptr_tree_node_allocator>
template <class iter>
iter ptr_tree<T, ptr_tree_node_allocator>::append_children(iter position, sibling_iterator from, sibling_iterator to)
	{
	assert(position.node!=head);
	assert(position.node);
	...
	while(from!=to) {
		insert_subptr_tree(position.end(), from);
		++from;
		}
	return ret;
	}
...
template <class T, class ptr_tree_node_allocator>
template <class iter>
iter ptr_tree<T, ptr_tree_node_allocator>::prepend_children(iter position, sibling_iterator from, sibling_iterator to)
	{
	assert(position.node!=head);
	assert(position.node);
...
template <class T, class ptr_tree_node_allocator>
typename ptr_tree<T, ptr_tree_node_allocator>::pre_order_iterator ptr_tree<T, ptr_tree_node_allocator>::set_head(const T& x)
	{
	assert(head->next_sibling==feet);
	return insert(iterator(feet), x);
	}
...
template <class T, class ptr_tree_node_allocator>
template <class iter>
iter ptr_tree<T, ptr_tree_node_allocator>::insert(iter position, const T& x)
	{
	if(position.node==0) {
		position.node=feet; // Backward compatibility: when calling insert on a null node,
		                    // insert before the feet.
...
template <class T, class ptr_tree_node_allocator>
typename ptr_tree<T, ptr_tree_node_allocator>::sibling_iterator ptr_tree<T, ptr_tree_node_allocator>::insert(sibling_iterator position, const T& x)
	{
	ptr_tree_node* tmp = alloc_.allocate(1,0);
	kp::constructor(&tmp->data, x);
	tmp->first_child=0;
	...
	tmp->next_sibling=position.node;
	if(position.node==0) { // iterator points to end of a subptr_tree
		tmp->parent=position.parent_;
		tmp->prev_sibling=position.range_last();
		tmp->parent->last_child=tmp;
		}
...
template <class T, class ptr_tree_node_allocator>
template <class iter>
iter ptr_tree<T, ptr_tree_node_allocator>::insert_after(iter position, const T& x)
	{
	ptr_tree_node* tmp = alloc_.allocate(1,0);
	kp::constructor(&tmp->data, x);
	tmp->first_child=0;
...
template <class T, class ptr_tree_node_allocator>
template <class iter>
iter ptr_tree<T, ptr_tree_node_allocator>::insert_subtree(iter position, const iterator_base& subtree)
	{
	// insert dummy
	iter it=insert(position, value_type());
	// replace dummy with subtree
...
template <class T, class ptr_tree_node_allocator>
template <class iter>
iter ptr_tree<T, ptr_tree_node_allocator>::insert_subtree_after(iter position, const iterator_base& subtree)
	{
	// insert dummy
	iter it=insert_after(position, value_type());
	// replace dummy with subtree
...
// template <class T, class ptr_tree_node_allocator>
// template <class iter>
// iter ptr_tree<T, ptr_tree_node_allocator>::insert_subtree(sibling_iterator position, iter subtree)
// 	{
// 	// insert dummy
// 	iter it(insert(position, value_type()));
// 	// replace dummy with subtree
...
template <class T, class ptr_tree_node_allocator>
template <class iter>
iter ptr_tree<T, ptr_tree_node_allocator>::replace(iter position, const T& x)
	{
	kp::ptr_destructor(&position.node->data);
	kp::constructor(&position.node->data, x);
	return position;
	}
...
template <class T, class ptr_tree_node_allocator>
template <class iter>
iter ptr_tree<T, ptr_tree_node_allocator>::replace(iter position, const iterator_base& from)
	{
	assert(position.node!=head);
	ptr_tree_node *current_from=from.node;
	ptr_tree_node *start_from=from.node;
	ptr_tree_node *current_to  =position.node;
...
//	std::cout << "no warning!" << std::endl;
	...
	tmp->next_sibling=current_to->next_sibling;
	tmp->parent=current_to->parent;
	kp::ptr_destructor(&current_to->data);
	alloc_.deallocate(current_to,1);
	current_to=tmp;
	
	// only at this stage can we fix 'last'
	ptr_tree_node *last=from.node->next_sibling;
...
template <class T, class ptr_tree_node_allocator>
typename ptr_tree<T, ptr_tree_node_allocator>::sibling_iterator ptr_tree<T, ptr_tree_node_allocator>::replace(
	sibling_iterator orig_begin, 
	sibling_iterator orig_end, 
	sibling_iterator new_begin, 
	sibling_iterator new_end)
	{
	ptr_tree_node *orig_first=orig_begin.node;
	ptr_tree_node *new_first=new_begin.node;
	ptr_tree_node *orig_last=orig_first;
	while((++orig_begin)!=orig_end)
		orig_last=orig_last->next_sibling;
	ptr_tree_node *new_last=new_first;
	while((++new_begin)!=new_end)
		new_last=new_last->next_sibling;
...
template <class T, class ptr_tree_node_allocator>
template <typename iter>
iter ptr_tree<T, ptr_tree_node_allocator>::flatten(iter position)
	{
	if(position.node->first_child==0)
		return position;

	ptr_tree_node *tmp=position.node->first_child;
	while(tmp) {
		tmp->parent=position.node->parent;
		tmp=tmp->next_sibling;
		} 
...
template <class T, class ptr_tree_node_allocator>
template <typename iter>
iter ptr_tree<T, ptr_tree_node_allocator>::reparent(iter position, sibling_iterator begin, sibling_iterator end)
	{
	ptr_tree_node *first=begin.node;
	ptr_tree_node *last=first;
...
template <class T, class ptr_tree_node_allocator>
template <typename iter> iter ptr_tree<T, ptr_tree_node_allocator>::reparent(iter position, iter from)
	{
	if(from.node->first_child==0) return position;
	return reparent(position, from.node->first_child, end(from));
	}
...
template <class T, class ptr_tree_node_allocator>
template <typename iter> iter ptr_tree<T, ptr_tree_node_allocator>::wrap(iter position, const T& x)
	{
	assert(position.node!=0);
	sibling_iterator fr=position, to=position;
	++to;
...
template <class T, class ptr_tree_node_allocator>
template <typename iter> iter ptr_tree<T, ptr_tree_node_allocator>::move_after(iter target, iter source)
   {
   ptr_tree_node *dst=target.node;
   ptr_tree_node *src=source.node;
   assert(dst);
   assert(src);
...
template <class T, class ptr_tree_node_allocator>
template <typename iter> iter ptr_tree<T, ptr_tree_node_allocator>::move_before(iter target, iter source)
   {
   ptr_tree_node *dst=target.node;
   ptr_tree_node *src=source.node;
   assert(dst);
...
// specialisation for sibling_iterators
template <class T, class ptr_tree_node_allocator>
typename ptr_tree<T, ptr_tree_node_allocator>::sibling_iterator ptr_tree<T, ptr_tree_node_allocator>::move_before(sibling_iterator target, 
																													  sibling_iterator source)
	{
	ptr_tree_node *dst=target.node;
	ptr_tree_node *src=source.node;
	ptr_tree_node *dst_prev_sibling;
	if(dst==0) { // must then be an end iterator
		dst_prev_sibling=target.parent_->last_child;
		assert(dst_prev_sibling);
		}
...
template <class T, class ptr_tree_node_allocator>
template <typename iter> iter ptr_tree<T, ptr_tree_node_allocator>::move_ontop(iter target, iter source)
	{
	ptr_tree_node *dst=target.node;
	ptr_tree_node *src=source.node;
	assert(dst);
	...
	// remember connection points
	ptr_tree_node *b_prev_sibling=dst->prev_sibling;
	ptr_tree_node *b_next_sibling=dst->next_sibling;
	ptr_tree_node *b_parent=dst->parent;
...
template <class T, class ptr_tree_node_allocator>
template <typename iter> iter ptr_tree<T, ptr_tree_node_allocator>::move_below(iter target, iter source)
	{
	ptr_tree_node *dst=target.node;
	ptr_tree_node *src=source.node;
	assert(dst);
...
template <class T, class ptr_tree_node_allocator>
void ptr_tree<T, ptr_tree_node_allocator>::merge(sibling_iterator to1,   sibling_iterator to2,
														sibling_iterator from1, sibling_iterator from2,
														bool duplicate_leaves)
	{
	sibling_iterator fnd;
...
template <class T, class ptr_tree_node_allocator>
void ptr_tree<T, ptr_tree_node_allocator>::sort(sibling_iterator from, sibling_iterator to, bool deep)
	{
	std::less<T> comp;
	sort(from, to, comp, deep);
	}
...
template <class T, class ptr_tree_node_allocator>
template <class StrictWeakOrdering>
void ptr_tree<T, ptr_tree_node_allocator>::sort(sibling_iterator from, sibling_iterator to, 
													 StrictWeakOrdering comp, bool deep)
	{
	if(from==to) return;
	// make list of sorted nodes
	// CHECK: if multiset stores equivalent nodes in the order in which they
	// are inserted, then this routine should be called 'stable_sort'.
	std::multiset<ptr_tree_node *, compare_nodes<StrictWeakOrdering> > nodes(comp);
	sibling_iterator it=from, it2=to;
	while(it != to) {
		nodes.insert(it.node);
		++it;
	...
	// prev and next are the nodes before and after the sorted range
	ptr_tree_node *prev=from.node->prev_sibling;
	ptr_tree_node *next=it2.node->next_sibling;
	typename std::multiset<ptr_tree_node *, compare_nodes<StrictWeakOrdering> >::iterator nit=nodes.begin(), eit=nodes.end();
	if(prev==0) {
		if((*nit)->parent!=0) // to catch "sorting the head" situations, when there is no parent
			(*nit)->parent->first_child=(*nit);
		}
...
template <class T, class ptr_tree_node_allocator>
template <typename iter>
bool ptr_tree<T, ptr_tree_node_allocator>::equal(const iter& one_, const iter& two, const iter& three_) const
	{
	std::equal_to<T> comp;
	return equal(one_, two, three_, comp);
	}
...
template <class T, class ptr_tree_node_allocator>
template <typename iter>
bool ptr_tree<T, ptr_tree_node_allocator>::equal_subtree(const iter& one_, const iter& two_) const
	{
	std::equal_to<T> comp;
	return equal_subtree(one_, two_, comp);
	}
...
template <class T, class ptr_tree_node_allocator>
template <typename iter, class BinaryPredicate>
bool ptr_tree<T, ptr_tree_node_allocator>::equal(const iter& one_, const iter& two, const iter& three_, BinaryPredicate fun) const
	{
	pre_order_iterator one(one_), three(three_);
...
template <class T, class ptr_tree_node_allocator>
template <typename iter, class BinaryPredicate>
bool ptr_tree<T, ptr_tree_node_allocator>::equal_subtree(const iter& one_, const iter& two_, BinaryPredicate fun) const
	{
	pre_order_iterator one(one_), two(two_);
...
template <class T, class ptr_tree_node_allocator>
ptr_tree<T, ptr_tree_node_allocator> ptr_tree<T, ptr_tree_node_allocator>::subtree(sibling_iterator from, sibling_iterator to) const
	{
	ptr_tree tmp;
	tmp.set_head(value_type());
	tmp.replace(tmp.begin(), tmp.end(), from, to);
...
template <class T, class ptr_tree_node_allocator>
void ptr_tree<T, ptr_tree_node_allocator>::subtree(ptr_tree& tmp, sibling_iterator from, sibling_iterator to) const
	{
	tmp.set_head(value_type());
	tmp.replace(tmp.begin(), tmp.end(), from, to);
	}
...
template <class T, class ptr_tree_node_allocator>
size_t ptr_tree<T, ptr_tree_node_allocator>::size() const
	{
	size_t i=0;
	pre_order_iterator it=begin(), eit=end();
	while(it!=eit) {
...
template <class T, class ptr_tree_node_allocator>
size_t ptr_tree<T, ptr_tree_node_allocator>::size(const iterator_base& top) const
	{
	size_t i=0;
	pre_order_iterator it=top, eit=top;
	eit.skip_children();
...
template <class T, class ptr_tree_node_allocator>
bool ptr_tree<T, ptr_tree_node_allocator>::empty() const
	{
	pre_order_iterator it=begin(), eit=end();
	return (it==eit);
	}
...
template <class T, class ptr_tree_node_allocator>
int ptr_tree<T, ptr_tree_node_allocator>::depth(const iterator_base& it) 
	{
	ptr_tree_node* pos=it.node;
	assert(pos!=0);
	int ret=0;
	while(pos->parent!=0) {
		pos=pos->parent;
...
template <class T, class ptr_tree_node_allocator>
int ptr_tree<T, ptr_tree_node_allocator>::depth(const iterator_base& it, const iterator_base& root) 
	{
	ptr_tree_node* pos=it.node;
	assert(pos!=0);
	int ret=0;
...
template <class T, class ptr_tree_node_allocator>
int ptr_tree<T, ptr_tree_node_allocator>::max_depth() const
	{
	int maxd=-1;
	for(ptr_tree_node *it = head->next_sibling; it!=feet; it=it->next_sibling)
		maxd=std::max(maxd, max_depth(it));
...
template <class T, class ptr_tree_node_allocator>
int ptr_tree<T, ptr_tree_node_allocator>::max_depth(const iterator_base& pos) const
	{
	ptr_tree_node *tmp=pos.node;
...
template <class T, class ptr_tree_node_allocator>
unsigned int ptr_tree<T, ptr_tree_node_allocator>::number_of_children(const iterator_base& it) 
	{
	ptr_tree_node *pos=it.node->first_child;
	if(pos==0) return 0;
...
template <class T, class ptr_tree_node_allocator>
unsigned int ptr_tree<T, ptr_tree_node_allocator>::number_of_siblings(const iterator_base& it) const
	{
	ptr_tree_node *pos=it.node;
	unsigned int ret=0;
	// count forward
...
template <class T, class ptr_tree_node_allocator>
void ptr_tree<T, ptr_tree_node_allocator>::swap(sibling_iterator it)
	{
	ptr_tree_node *nxt=it.node->next_sibling;
	if(nxt) {
		if(it.node->prev_sibling)
			it.node->prev_sibling->next_sibling=nxt;
		else
			it.node->parent->first_child=nxt;
		nxt->prev_sibling=it.node->prev_sibling;
		ptr_tree_node *nxtnxt=nxt->next_sibling;
		if(nxtnxt)
			nxtnxt->prev_sibling=it.node;
		else
			it.node->parent->last_child=it.node;
...
template <class T, class ptr_tree_node_allocator>
void ptr_tree<T, ptr_tree_node_allocator>::swap(iterator one, iterator two)
	{
	// if one and two are adjacent siblings, use the sibling swap
	if(one.node->next_sibling==two.node) swap(one);
	else if(two.node->next_sibling==one.node) swap(two);
	else {
		ptr_tree_node *nxt1=one.node->next_sibling;
		ptr_tree_node *nxt2=two.node->next_sibling;
		ptr_tree_node *pre1=one.node->prev_sibling;
		ptr_tree_node *pre2=two.node->prev_sibling;
		ptr_tree_node *par1=one.node->parent;
		ptr_tree_node *par2=two.node->parent;
...
// template <class BinaryPredicate>
// ptr_tree<T, ptr_tree_node_allocator>::iterator ptr_tree<T, ptr_tree_node_allocator>::find_subtree(
// 	sibling_iterator subfrom, sibling_iterator subto, iterator from, iterator to, 
// 	BinaryPredicate fun) const
// 	{
// 	assert(1==0); // this routine is not finished yet.
...
template <class T, class ptr_tree_node_allocator>
bool ptr_tree<T, ptr_tree_node_allocator>::is_in_subtree(const iterator_base& it, const iterator_base& begin, 
																 const iterator_base& end) const
	{
	// FIXME: this should be optimised.
	pre_order_iterator tmp=begin;
...
template <class T, class ptr_tree_node_allocator>
bool ptr_tree<T, ptr_tree_node_allocator>::is_valid(const iterator_base& it) const
	{
	if(it.node==0 || it.node==feet || it.node==head) return false;
	else return true;
	}
...
template <class T, class ptr_tree_node_allocator>
unsigned int ptr_tree<T, ptr_tree_node_allocator>::index(sibling_iterator it) const
	{
	unsigned int ind=0;
	if(it.node->parent==0) {
		while(it.node->prev_sibling!=head) {
...
template <class T, class ptr_tree_node_allocator>
typename ptr_tree<T, ptr_tree_node_allocator>::sibling_iterator ptr_tree<T, ptr_tree_node_allocator>::sibling(const iterator_base& it, unsigned int num)
   {
   ptr_tree_node *tmp;
   if(it.node->parent==0) {
      tmp=head->next_sibling;
...
template <class T, class ptr_tree_node_allocator>
typename ptr_tree<T, ptr_tree_node_allocator>::sibling_iterator ptr_tree<T, ptr_tree_node_allocator>::child(const iterator_base& it, unsigned int num) 
	{
	ptr_tree_node *tmp=it.node->first_child;
	while(num--) {
		assert(tmp!=0);
		tmp=tmp->next_sibling;
		}
...
template <class T, class ptr_tree_node_allocator>
ptr_tree<T, ptr_tree_node_allocator>::iterator_base::iterator_base()
	: node(0), skip_current_children_(false)
	{
	}

template <class T, class ptr_tree_node_allocator>
ptr_tree<T, ptr_tree_node_allocator>::iterator_base::iterator_base(ptr_tree_node *tn)
	: node(tn), skip_current_children_(false)
	{
	}

template <class T, class ptr_tree_node_allocator>
T& ptr_tree<T, ptr_tree_node_allocator>::iterator_base::operator*() const
	{
	return node->data;
	}
...
template <class T, class ptr_tree_node_allocator>
bool ptr_tree<T, ptr_tree_node_allocator>::post_order_iterator::operator!=(const post_order_iterator& other) const
	{
	if(other.node!=this->node) return true;
	else return false;
	}
...
template <class T, class ptr_tree_node_allocator>
bool ptr_tree<T, ptr_tree_node_allocator>::pre_order_iterator::operator!=(const pre_order_iterator& other) const
	{
	if(other.node!=this->node) return true;
	else return false;
	}
...
template <class T, class ptr_tree_node_allocator>
bool ptr_tree<T, ptr_tree_node_allocator>::sibling_iterator::operator!=(const sibling_iterator& other) const
	{
	if(other.node!=this->node) return true;
	else return false;
	}
...
template <class T, class ptr_tree_node_allocator>
bool ptr_tree<T, ptr_tree_node_allocator>::leaf_iterator::operator!=(const leaf_iterator& other) const
   {
   if(other.node!=this->node) return true;
   else return false;
   }
...
template <class T, class ptr_tree_node_allocator>
typename ptr_tree<T, ptr_tree_node_allocator>::sibling_iterator ptr_tree<T, ptr_tree_node_allocator>::iterator_base::begin() const
	{
	if(node->first_child==0) 
		return end();
...
template <class T, class ptr_tree_node_allocator>
typename ptr_tree<T, ptr_tree_node_allocator>::sibling_iterator ptr_tree<T, ptr_tree_node_allocator>::iterator_base::end() const
	{
	sibling_iterator ret(0);
	ret.parent_=node;
	return ret;
...
template <class T, class ptr_tree_node_allocator>
void ptr_tree<T, ptr_tree_node_allocator>::iterator_base::skip_children()
	{
	skip_current_children_=true;
	}

template <class T, class ptr_tree_node_allocator>
void ptr_tree<T, ptr_tree_node_allocator>::iterator_base::skip_children(bool skip)
   {
   skip_current_children_=skip;
   }

template <class T, class ptr_tree_node_allocator>
unsigned int ptr_tree<T, ptr_tree_node_allocator>::iterator_base::number_of_children() const
	{
	ptr_tree_node *pos=node->first_child;
	if(pos==0) return 0;
...
template <class T, class ptr_tree_node_allocator>
ptr_tree<T, ptr_tree_node_allocator>::pre_order_iterator::pre_order_iterator() 
	: iterator_base(0)
	{
	}

template <class T, class ptr_tree_node_allocator>
ptr_tree<T, ptr_tree_node_allocator>::pre_order_iterator::pre_order_iterator(ptr_tree_node *tn)
	: iterator_base(tn)
	{
	}

template <class T, class ptr_tree_node_allocator>
ptr_tree<T, ptr_tree_node_allocator>::pre_order_iterator::pre_order_iterator(const iterator_base &other)
	: iterator_base(other.node)
	{
	}

template <class T, class ptr_tree_node_allocator>
ptr_tree<T, ptr_tree_node_allocator>::pre_order_iterator::pre_order_iterator(const sibling_iterator& other)
	: iterator_base(other.node)
	{
	if(this->node==0) {
		if(other.range_last()!=0)
...
template <class T, class ptr_tree_node_allocator>
typename ptr_tree<T, ptr_tree_node_allocator>::pre_order_iterator& ptr_tree<T, ptr_tree_node_allocator>::pre_order_iterator::operator++()
	{
	assert(this->node!=0);
	if(!this->skip_current_children_ && this->node->first_child != 0) {
		this->node=this->node->first_child;
...
template <class T, class ptr_tree_node_allocator>
typename ptr_tree<T, ptr_tree_node_allocator>::pre_order_iterator ptr_tree<T, ptr_tree_node_allocator>::pre_order_iterator::operator++(int)
	{
	pre_order_iterator copy = *this;
	++(*this);
	return copy;
...
template <class T, class ptr_tree_node_allocator>
typename ptr_tree<T, ptr_tree_node_allocator>::pre_order_iterator& ptr_tree<T, ptr_tree_node_allocator>::pre_order_iterator::operator+=(unsigned int num)
	{
	while(num>0) {
		++(*this);
		--num;
...
template <class T, class ptr_tree_node_allocator>
ptr_tree<T, ptr_tree_node_allocator>::post_order_iterator::post_order_iterator() 
	: iterator_base(0)
	{
	}

template <class T, class ptr_tree_node_allocator>
ptr_tree<T, ptr_tree_node_allocator>::post_order_iterator::post_order_iterator(ptr_tree_node *tn)
	: iterator_base(tn)
	{
	}

template <class T, class ptr_tree_node_allocator>
ptr_tree<T, ptr_tree_node_allocator>::post_order_iterator::post_order_iterator(const iterator_base &other)
	: iterator_base(other.node)
	{
	}

template <class T, class ptr_tree_node_allocator>
ptr_tree<T, ptr_tree_node_allocator>::post_order_iterator::post_order_iterator(const sibling_iterator& other)
	: iterator_base(other.node)
	{
	if(this->node==0) {
		if(other.range_last()!=0)
...
template <class T, class ptr_tree_node_allocator>
typename ptr_tree<T, ptr_tree_node_allocator>::post_order_iterator& ptr_tree<T, ptr_tree_node_allocator>::post_order_iterator::operator++()
	{
	assert(this->node!=0);
	if(this->node->next_sibling==0) {
		this->node=this->node->parent;
...
template <class T, class ptr_tree_node_allocator>
typename ptr_tree<T, ptr_tree_node_allocator>::post_order_iterator ptr_tree<T, ptr_tree_node_allocator>::post_order_iterator::operator++(int)
	{
	post_order_iterator copy = *this;
	++(*this);
	return copy;
...
template <class T, class ptr_tree_node_allocator>
typename ptr_tree<T, ptr_tree_node_allocator>::post_order_iterator& ptr_tree<T, ptr_tree_node_allocator>::post_order_iterator::operator+=(unsigned int num)
	{
	while(num>0) {
		++(*this);
		--num;
...
template <class T, class ptr_tree_node_allocator>
void ptr_tree<T, ptr_tree_node_allocator>::post_order_iterator::descend_all()
	{
	assert(this->node!=0);
	while(this->node->first_child)
		this->node=this->node->first_child;
...
template <class T, class ptr_tree_node_allocator>
ptr_tree<T, ptr_tree_node_allocator>::breadth_first_queued_iterator::breadth_first_queued_iterator()
	: iterator_base()
	{
	}

template <class T, class ptr_tree_node_allocator>
ptr_tree<T, ptr_tree_node_allocator>::breadth_first_queued_iterator::breadth_first_queued_iterator(ptr_tree_node *tn)
	: iterator_base(tn)
	{
	traversal_queue.push(tn);
	}
...
template <class T, class ptr_tree_node_allocator>
ptr_tree<T, ptr_tree_node_allocator>::breadth_first_queued_iterator::breadth_first_queued_iterator(const iterator_base& other)
	: iterator_base(other.node)
	{
	traversal_queue.push(other.node);
	}
...
template <class T, class ptr_tree_node_allocator>
bool ptr_tree<T, ptr_tree_node_allocator>::breadth_first_queued_iterator::operator!=(const breadth_first_queued_iterator& other) const
	{
	if(other.node!=this->node) return true;
	else return false;
	}
...
template <class T, class ptr_tree_node_allocator>
typename ptr_tree<T, ptr_tree_node_allocator>::breadth_first_queued_iterator& ptr_tree<T, ptr_tree_node_allocator>::breadth_first_queued_iterator::operator++()
	{
	assert(this->node!=0);
...
template <class T, class ptr_tree_node_allocator>
typename ptr_tree<T, ptr_tree_node_allocator>::breadth_first_queued_iterator ptr_tree<T, ptr_tree_node_allocator>::breadth_first_queued_iterator::operator++(int)
	{
	breadth_first_queued_iterator copy = *this;
	++(*this);
	return copy;
...
template <class T, class ptr_tree_node_allocator>
typename ptr_tree<T, ptr_tree_node_allocator>::breadth_first_queued_iterator& ptr_tree<T, ptr_tree_node_allocator>::breadth_first_queued_iterator::operator+=(unsigned int num)
	{
	while(num>0) {
		++(*this);
		--num;
...
template <class T, class ptr_tree_node_allocator>
ptr_tree<T, ptr_tree_node_allocator>::fixed_depth_iterator::fixed_depth_iterator()
	: iterator_base()
	{
	}

template <class T, class ptr_tree_node_allocator>
ptr_tree<T, ptr_tree_node_allocator>::fixed_depth_iterator::fixed_depth_iterator(ptr_tree_node *tn)
	: iterator_base(tn), top_node(0)
	{
	}

template <class T, class ptr_tree_node_allocator>
ptr_tree<T, ptr_tree_node_allocator>::fixed_depth_iterator::fixed_depth_iterator(const iterator_base& other)
	: iterator_base(other.node), top_node(0)
	{
	}

template <class T, class ptr_tree_node_allocator>
ptr_tree<T, ptr_tree_node_allocator>::fixed_depth_iterator::fixed_depth_iterator(const sibling_iterator& other)
	: iterator_base(other.node), top_node(0)
	{
	}

template <class T, class ptr_tree_node_allocator>
ptr_tree<T, ptr_tree_node_allocator>::fixed_depth_iterator::fixed_depth_iterator(const fixed_depth_iterator& other)
	: iterator_base(other.node), top_node(other.top_node)
	{
	}

template <class T, class ptr_tree_node_allocator>
bool ptr_tree<T, ptr_tree_node_allocator>::fixed_depth_iterator::operator==(const fixed_depth_iterator& other) const
	{
	if(other.node==this->node && other.top_node==top_node) return true;
	else return false;
	}
...
template <class T, class ptr_tree_node_allocator>
typename ptr_tree<T, ptr_tree_node_allocator>::fixed_depth_iterator& ptr_tree<T, ptr_tree_node_allocator>::fixed_depth_iterator::operator++()
	{
	assert(this->node!=0);
...
//		}
//	else {
//		ptr_tree_node *par=this->node->parent;
//		do {
//			par=par->prev_sibling;
//			if(par==0) { // FIXME: need to keep track of this!
//				this->node=0;
...
template <class T, class ptr_tree_node_allocator>
typename ptr_tree<T, ptr_tree_node_allocator>::fixed_depth_iterator ptr_tree<T, ptr_tree_node_allocator>::fixed_depth_iterator::operator++(int)
	{
	fixed_depth_iterator copy = *this;
	++(*this);
	return copy;
...
template <class T, class ptr_tree_node_allocator>
typename ptr_tree<T, ptr_tree_node_allocator>::fixed_depth_iterator& ptr_tree<T, ptr_tree_node_allocator>::fixed_depth_iterator::operator-=(unsigned int num)
	{
	while(num>0) {
		--(*this);
		--(num);
...
template <class T, class ptr_tree_node_allocator>
ptr_tree<T, ptr_tree_node_allocator>::sibling_iterator::sibling_iterator() 
	: iterator_base()
	{
	set_parent_();
	}
...
template <class T, class ptr_tree_node_allocator>
ptr_tree<T, ptr_tree_node_allocator>::sibling_iterator::sibling_iterator(ptr_tree_node *tn)
	: iterator_base(tn)
	{
	set_parent_();
	}
...
template <class T, class ptr_tree_node_allocator>
ptr_tree<T, ptr_tree_node_allocator>::sibling_iterator::sibling_iterator(const iterator_base& other)
	: iterator_base(other.node)
	{
	set_parent_();
	}
...
template <class T, class ptr_tree_node_allocator>
ptr_tree<T, ptr_tree_node_allocator>::sibling_iterator::sibling_iterator(const sibling_iterator& other)
	: iterator_base(other), parent_(other.parent_)
	{
	}

template <class T, class ptr_tree_node_allocator>
void ptr_tree<T, ptr_tree_node_allocator>::sibling_iterator::set_parent_()
	{
	parent_=0;
	if(this->node==0) return;
	if(this->node->parent!=0)
...
template <class T, class ptr_tree_node_allocator>
typename ptr_tree<T, ptr_tree_node_allocator>::sibling_iterator& ptr_tree<T, ptr_tree_node_allocator>::sibling_iterator::operator++()
	{
	if(this->node)
		this->node=this->node->next_sibling;
	return *this;
...
template <class T, class ptr_tree_node_allocator>
typename ptr_tree<T, ptr_tree_node_allocator>::sibling_iterator ptr_tree<T, ptr_tree_node_allocator>::sibling_iterator::operator++(int)
	{
	sibling_iterator copy = *this;
	++(*this);
	return copy;
...
template <class T, class ptr_tree_node_allocator>
typename ptr_tree<T, ptr_tree_node_allocator>::sibling_iterator& ptr_tree<T, ptr_tree_node_allocator>::sibling_iterator::operator+=(unsigned int num)
	{
	while(num>0) {
		++(*this);
		--num;
...
template <class T, class ptr_tree_node_allocator>
typename ptr_tree<T, ptr_tree_node_allocator>::ptr_tree_node *ptr_tree<T, ptr_tree_node_allocator>::sibling_iterator::range_first() const
	{
	ptr_tree_node *tmp=parent_->first_child;
	return tmp;
	}

template <class T, class ptr_tree_node_allocator>
typename ptr_tree<T, ptr_tree_node_allocator>::ptr_tree_node *ptr_tree<T, ptr_tree_node_allocator>::sibling_iterator::range_last() const
	{
	return parent_->last_child;
	}
...
template <class T, class ptr_tree_node_allocator>
ptr_tree<T, ptr_tree_node_allocator>::leaf_iterator::leaf_iterator() 
   : iterator_base(0), top_node(0)
   {
   }

template <class T, class ptr_tree_node_allocator>
ptr_tree<T, ptr_tree_node_allocator>::leaf_iterator::leaf_iterator(ptr_tree_node *tn, ptr_tree_node *top)
   : iterator_base(tn), top_node(top)
   {
   }

template <class T, class ptr_tree_node_allocator>
ptr_tree<T, ptr_tree_node_allocator>::leaf_iterator::leaf_iterator(const iterator_base &other)
   : iterator_base(other.node), top_node(0)
   {
   }

template <class T, class ptr_tree_node_allocator>
ptr_tree<T, ptr_tree_node_allocator>::leaf_iterator::leaf_iterator(const sibling_iterator& other)
   : iterator_base(other.node), top_node(0)
   {
   if(this->node==0) {
      if(other.range_last()!=0)
...
template <class T, class ptr_tree_node_allocator>
typename ptr_tree<T, ptr_tree_node_allocator>::leaf_iterator& ptr_tree<T, ptr_tree_node_allocator>::leaf_iterator::operator++()
   {
	assert(this->node!=0);
	if(this->node->first_child!=0) { // current node is no longer leaf (children got added)
		 while(this->node->first_child) 
...
template <class T, class ptr_tree_node_allocator>
typename ptr_tree<T, ptr_tree_node_allocator>::leaf_iterator ptr_tree<T, ptr_tree_node_allocator>::leaf_iterator::operator++(int)
   {
   leaf_iterator copy = *this;
   ++(*this);
   return copy;
...
template <class T, class ptr_tree_node_allocator>
typename ptr_tree<T, ptr_tree_node_allocator>::leaf_iterator& ptr_tree<T, ptr_tree_node_allocator>::leaf_iterator::operator+=(unsigned int num)
   {
   while(num>0) {
      ++(*this);
      --num;
