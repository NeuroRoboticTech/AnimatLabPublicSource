...
template<class T>
void ptr_print_tree_bracketed(const tree<T>& t, std::ostream& str);

template<class T>
void ptr_print_subtree_bracketed(const tree<T>& t, typename tree<T>::iterator iRoot, 
									  std::ostream& str=std::cout);
...
void ptr_print_tree_bracketed(const tree<T>& t, std::ostream& str) 
	...
	int headNum = 0;
	for(typename tree<T>::sibling_iterator iRoots = t.begin(); iRoots != t.end(); ++iRoots, ++headNum) {
		ptr_print_subtree_bracketed(t,iRoots,str);
		if (headNum != headCount) {
			str << std::endl;
			}
		}
...
template<class T>
void ptr_print_subtree_bracketed(const tree<T>& t, typename tree<T>::iterator iRoot, std::ostream& str) 
	{
	if(t.empty()) return;
	if (t.number_of_children(iRoot) == 0) 
	{
		T lpVal = (T) iRoot.node->data;
		lpVal->operator<<(str);
	}
	else {
		// parent
		...
		for (iChildren = t.begin(iRoot), siblingNum = 0; iChildren != t.end(iRoot); ++iChildren, ++siblingNum) {
			// recursively print child
			ptr_print_subtree_bracketed(t,iChildren,str);
			// comma after every child except the last one
			if (siblingNum != siblingCount ) {
				str << ", ";
				}
