...
public:
	...
	CStdStr(const int iVal)
	{
		Format("%d", iVal);
	}

	CStdStr(const long lVal)
	{
		Format("%d", lVal);
	}

	CStdStr(const float fltVal)
	{
		Format("%f", fltVal);
	}

	CStdStr(const double dblVal)
	{
		Format("%f", dblVal);
	}
	#ifdef SS_INC_COMDEF
...
//		}
	...
	BOOL IsNumeric()
	{
		int iPos = find_first_not_of("0123456789.+-eE");
		if(iPos == -1)
			return true;
		else
			return false;
	...
	BOOL IsIntegerType()
	{
		int iPos = find_first_not_of("0123456789+-");
		if(iPos == -1)
			return true;
		else
			return false;
