...
OsgCameraManipulator::OsgCameraManipulator(Simulator *lpSim, osgViewer::Viewer *osgViewer, osg::Viewport *osgViewport)
{
    ...
    this->setMinimumDistance(0.00001);
}
...
bool OsgCameraManipulator::handle(const GUIEventAdapter& ea, GUIActionAdapter& aa)
{
	bool bHandled = false;
...
const osg::Camera *OsgCameraManipulator::GetCamera(float x, float y, float &local_x, float &local_y)
{
...
const osg::Camera *OsgCameraManipulator::GetCamera(float x, float y)
{
...
void OsgCameraManipulator::pick(const osgGA::GUIEventAdapter& ea, GUIActionAdapter& aa)
{
...
#pragma region LineSegmentIntersector
        ...
        {	
			...
			{
				//If the user data is a OsgUserData object then lets use it.
				OsgUserData *osgData = dynamic_cast<OsgUserData *>(hitr->drawable->getUserData());				
...
osg::Vec3 OsgCameraManipulator::FindSelectedVertex(osgUtil::LineSegmentIntersector::Intersections::iterator &hitr)
{
	...
	if(vil.size() > 0 && osgGeom)
	{
		...
		if(vil.size() != ral.size())
			THROW_ERROR(Osg_Err_lHitArrayMismatch, Osg_Err_strHitArrayMismatch);
...
/*
void OsgCameraManipulator::GetPickedFace(int iIndex, osg::Geometry *osgGeo)
{
...
{	
	...
	m_lpSim->MouseSpringForceMagnitude(vTotalForce.length(), true);
	m_lpSim->MouseSpringDampingForceMagnitude(v3DampForce.length(), true);
	m_lpSim->MouseSpringLengthMagnitude(vSpringLength.length(), true);
...
void OsgCameraManipulator::DoPan(const GUIEventAdapter& ea, float x, float y)
{
...
void OsgCameraManipulator::DoZoom(const GUIEventAdapter& ea, float x, float y)
{	
	float dy = y - m_fltPrevY;	
...
void OsgCameraManipulator::DoRotate(const GUIEventAdapter& ea, float x, float y)
{	
	float dx = x - m_fltPrevX;
	float dy = y - m_fltPrevY;	
