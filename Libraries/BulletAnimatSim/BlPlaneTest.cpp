...
BlPlaneTest::~BlPlaneTest()
{
	...
	//}
	//catch(...)
	//{Std_TraceMsg(0, "Caught Error in desctructor of BlPlaneTest/\r\n", "", -1, false, true);}
}

void BlPlaneTest::CreateGraphicsGeometry()
{
	//m_osgGeometry = CreatePlaneGeometry(CornerX(), CornerY(), m_ptSize.x, m_ptSize.y, GridX(), GridY(), false);
}

void BlPlaneTest::CreatePhysicsGeometry()
{
	//if(IsCollisionObject())
	//	m_vxGeometry = new VxPlane();
}

void BlPlaneTest::CreateParts()
{ 
...
void BlPlaneTest::ResizePhysicsGeometry()
{
	//if(m_vxGeometry)
	//	m_vxGeometry = new VxPlane();
}
//Planes can never have fluid interactions/dynamics.
void BlPlaneTest::Physics_FluidDataChanged()
{}
