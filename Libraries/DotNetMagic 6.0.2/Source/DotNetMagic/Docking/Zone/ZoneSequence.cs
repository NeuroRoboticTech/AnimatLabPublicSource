...
namespace Crownwood.DotNetMagic.Docking
{
    ...
    public class ZoneSequence : Zone, IHotZoneSource, IZoneMaximizeWindow, IResizeSource
    {
        ...
        /// </summary>
        public override void RelayoutContents()
        {
            RepositionControls();
        }
        ...
        {
            bool ret = DockingManager.AllowResize;
        ...
        {
            bool wasShowingBar = NeedResizeBar(_oldStyle);
            bool needShowingBar = NeedResizeBar(this.Dock);

            // Is there a change in resize bar visibility needed?
            if (wasShowingBar != needShowingBar)
            {
                int barLength = _resizeBar.Length;

                // Expand to create room for the resize bar
                if (needShowingBar)
                {
                    if (_resizeBar.Direction == LayoutDirection.Horizontal)
                        Width += barLength;
                    else
                        Height += barLength;
                }
                ...
                {
                    // Contract as bar no longer needed
                    if (_resizeBar.Direction == LayoutDirection.Horizontal)
                        Width -= barLength;
                    else
                        Height -= barLength;
        ...
        private bool NeedResizeBar(DockStyle style)
        {
            switch (style)
            {
                case DockStyle.None:
