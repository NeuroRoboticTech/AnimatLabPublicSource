...
CStdAvi::CStdAvi(std::string const& file_name, int frame_period, const WAVEFORMATEX* wfx) { 
  IAVIFile *pfile;
...
HRESULT CStdAvi::compression(CStdBitmap const& bmp, AVICOMPRESSOPTIONS *opts, bool ShowDialog, HWND hparent) { 
  if (avi_==0) return AVIERR_BADHANDLE;

  DIBSECTION dibs; 
  //::GetObject(bmp,sizeof(dibs),&dibs);
  printf("CStdAvi::compression going to GetObject 1\n");
  if( ! ::GetObject(bmp, sizeof(dibs), &dibs)) {
    printf("GetObject: %s\n", Win32_LastError().c_str());
    //return -1;
  }
  printf("CStdAvi::compression went to GetObject 1\n");
  ...
  if (au->psCompressed==0) { 
    ...
    ::AVISaveOptionsFree(1,aopts);
    printf("CStdAvi::compression after AVISaveOptionsFree\n");

    /*
    DIBSECTION dibs; 
    printf("CStdAvi::compression going to GetObject\n");
    if( !::GetObject(bmp, sizeof(dibs), &dibs)) {
      printf("GetObject: %s\n", Win32_LastError());
      return -1;
    }
...
HRESULT CStdAvi::AddWindowFrame(HWND hWnd, BOOL bSaveImage, string strFile)
{
	...
	if(bSaveImage)
	{
		file.open(strFile.c_str(), ios::binary);
		if(file)
		{
	...
	CStdBitmap bmp(bitmap);

	if(m_bFirstFrame)
	{
		compression(bmp, &m_aviOpts, false, 0);
		m_bFirstFrame = FALSE;
	}

	HRESULT hr = add_frame(bmp);
...
HRESULT CStdAvi::add_frame(CStdBitmap const& bmp) { 
  if (avi_ == 0) return AVIERR_BADHANDLE;
  ...
  // TODO... Get Width and Height from CStdBitmap...
  if( au->ps == 0 ) { 
    AVISTREAMINFO strhdr; 
    ::ZeroMemory(&strhdr,sizeof(strhdr));
...
HRESULT CStdAvi::add_audio(void *dat, unsigned long numbytes) { 
  if (avi_==0) return AVIERR_BADHANDLE;
...
HRESULT CStdAvi::add_wav(const char* src, DWORD flags) {
  if (avi_==0) return AVIERR_BADHANDLE;
