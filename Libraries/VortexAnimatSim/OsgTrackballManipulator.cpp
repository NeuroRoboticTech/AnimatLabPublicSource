...
#include "OsgTrackballManipulator.h"
...
void OsgTrackballManipulator::setNode(osg::Node* node)
{
    _node = node;
    if (_node.get())
    {
...
const osg::Node* OsgTrackballManipulator::getNode() const
{
    return _node.get();
}


osg::Node* OsgTrackballManipulator::getNode()
{
    return _node.get();
}


void OsgTrackballManipulator::home(double /*currentTime*/)
{
    //osg::Vec3d vOrigEye = _homeEye;
    //osg::Vec3d vOrigCenter = _homeCenter;
    //osg::Vec3d vOrigUp = _homeUp;

    if (getAutoComputeHomePosition()) computeHomePosition();
    computePosition(_homeEye, _homeCenter, _homeEye);
    _thrown = false;
}

void OsgTrackballManipulator::home(const GUIEventAdapter& ea ,GUIActionAdapter& us)
{
    home(ea.getTime());
    us.requestRedraw();
...
void OsgTrackballManipulator::init(const GUIEventAdapter& ,GUIActionAdapter& )
{
    flushMouseEventStack();
}


void OsgTrackballManipulator::getUsage(osg::ApplicationUsage& usage) const
{
...
bool OsgTrackballManipulator::handle(const GUIEventAdapter& ea,GUIActionAdapter& us)
{
...
bool OsgTrackballManipulator::isMouseMoving()
{
...
void OsgTrackballManipulator::flushMouseEventStack()
{
    _ga_t1 = NULL;
    _ga_t0 = NULL;
}
...
void OsgTrackballManipulator::addMouseEvent(const GUIEventAdapter& ea)
{
    _ga_t1 = _ga_t0;
    _ga_t0 = &ea;
}

void OsgTrackballManipulator::setByMatrix(const osg::Matrixd& matrix)
{
    _center = osg::Vec3(0.0f,0.0f,-_distance)*matrix;
    _rotation = matrix.getRotate();
}

osg::Matrixd OsgTrackballManipulator::getMatrix() const
{
    return osg::Matrixd::translate(0.0,0.0,_distance)*osg::Matrixd::rotate(_rotation)*osg::Matrixd::translate(_center);
}

osg::Matrixd OsgTrackballManipulator::getInverseMatrix() const
{
    return osg::Matrixd::translate(-_center)*osg::Matrixd::rotate(_rotation.inverse())*osg::Matrixd::translate(0.0,0.0,-_distance);
}

void OsgTrackballManipulator::computePosition(const osg::Vec3& eye,const osg::Vec3& center,const osg::Vec3& up)
{
...
bool OsgTrackballManipulator::calcMovement()
{
 ...
 * Programmer.
 */
void OsgTrackballManipulator::setTrackballSize(float size)
{
    _trackballSize = size;
     osg::clampBetweenRange(_trackballSize,0.1f,1.0f,"OsgTrackballManipulator::setTrackballSize(float)");
}
 ...
 * (-1.0 ... 1.0)
 */
void OsgTrackballManipulator::trackball(osg::Vec3& axis,float& angle, float p1x, float p1y, float p2x, float p2y)
{
    /*
 ...
 */
float OsgTrackballManipulator::tb_project_to_sphere(float r, float x, float y)
{
    float d, t, z;
